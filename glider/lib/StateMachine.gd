...
# Registers the given state with the state machine and adds it as a child if it
# is not already part of the scene tree.
...
func remove_state(state_id: String) -> void:
    if state_id != null and state_id.length() > 0 and _states.has(state_id):
